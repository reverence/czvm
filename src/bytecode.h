...
#define ALL_CODE_LEN				0xff

typedef int (*interpreter_func)(u2 code_len, char *code_name, void *info); //指向具体执行的函数

typedef struct BYTE_CODE {
	u2 code;//jvm指令代码
	u2 code_len;//指令长度,反编译后看控制台输出即可知道长度
	char code_name[32];//指令名称
	interpreter_func func;//执行指令的函数,暂时使用指向函数的指针方式，后续有更好的方式在修改
} BYTE_CODE;
...
/////////////////////////////////////////////jvm byte code////////////////////////////////////////////////////////////////
BYTE_CODE jvm_byte_code[ALL_CODE_LEN]={
		                ...
		                {0xb4,  3,      "getfield",             jvm_interp_getfield},
		                {0xb5,  3,      "putfield",             jvm_interp_putfield},
